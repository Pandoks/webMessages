...
<div class="flex h-screen">
  <!-- Sidebar -->
  <div
    class="w-80 shrink-0 {hasChatSelected ? 'hidden md:block' : ''}
		md:w-80 lg:w-96"
  >
  ...
  {#if connection.isOffline}
    <div
      class="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 rounded-full bg-red-500 px-3 py-1 text-xs font-medium text-white shadow-lg"
    >
      Offline — showing cached data
    </div>
  {:else if !connection.connected}
    <div
      class="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 rounded-full bg-yellow-500 px-3 py-1 text-xs font-medium text-white shadow-lg"
    >
      Reconnecting...
    </div>
  {/if}
