  ...
  const job = (async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          'Video transcode with audio failed, retrying without audio track:',
          errWithAudio
        );
...
function parseByteRange(
  ...
  if (startStr) {
    ...
    if (!Number.isFinite(suffixLength) || Number.isNaN(suffixLength) || suffixLength <= 0)
      return null;
...
export const GET: RequestHandler = async ({ params, request, url }) => {
  ...
  let downloadName = sanitizeFilename(
    attachment.transfer_name ?? basename(filePath) ?? `attachment-${attachment.rowid}`
  );
