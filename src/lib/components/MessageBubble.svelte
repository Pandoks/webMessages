<script lang="ts">
	...
	const isRetracted = $derived(
		message.dateRetracted !== null || (message.text === null && message.dateEdited !== null)
	);
	...
	const canEdit = $derived(message.editExpiresAt != null && message.editExpiresAt > now);
	const canUnsend = $derived(message.unsendExpiresAt != null && message.unsendExpiresAt > now);
...
>
	<div class="flex max-w-[75%] min-w-0 flex-col {isSent ? 'items-end' : 'items-start'}">
		...
		{#if message.threadOriginatorGuid && truncatedReplyText}
			<div class="mb-1 flex items-start gap-1.5 px-1">
				<div
					class="w-0.5 shrink-0 self-stretch rounded-full {isSent
						? 'bg-blue-300'
						: 'bg-gray-400 dark:bg-gray-500'}"
				></div>
				<p
					class="truncate text-xs italic {isSent
						? 'text-blue-200'
		...
		{:else}
			...
			>
				...
				{:else}
						...
						<div class="flex flex-col gap-1.5">
							{#each displayAttachments as att (att.guid)}
								{#if isImage(att.mimeType)}
									<a href={attachmentUrl(att.guid)} target="_blank" rel="noopener noreferrer">
										<img
								...
								{:else if isVideo(att.mimeType)}
									...
									<video controls preload="metadata" class="max-h-64 max-w-full rounded-lg">
					...
					{#if displayText}
						<p
							class="text-sm [overflow-wrap:anywhere] whitespace-pre-wrap {isScheduled
...
{#if contextMenuVisible}
	...
	>
		...
		<div
			class="min-w-[160px] overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-black/10 dark:bg-gray-800 dark:ring-white/10"
		>
			{#if !isScheduled}
				<button
					onclick={handleReply}
				...
				>
					...
					>
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
							d="M3 10h10a5 5 0 015 5v4M3 10l6-6M3 10l6 6"
						/>
					</svg>
					Reply
				</button>
			...
			{:else if isSent && !isRetracted}
				...
				{#if displayText}
					...
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-4 w-4 {canEdit ? 'text-gray-400' : 'text-gray-300 dark:text-gray-600'}"
							fill="none"
							viewBox="0 0 24 24"
							stroke="currentColor"
							stroke-width="2"
						...
						Edit{#if canEdit}{@const t = formatTimeLeft(message.editExpiresAt)}{#if t}
								<span class="ml-auto text-xs text-gray-400 dark:text-gray-500">{t}</span
								>{/if}{:else}
							<span class="ml-auto text-xs text-gray-400 dark:text-gray-600">(expired)</span>{/if}
					</button>
				{/if}
				<button
				...
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="h-4 w-4 {canUnsend ? '' : 'text-gray-300 dark:text-gray-600'}"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
						stroke-width="2"
					...
					Unsend{#if canUnsend}{@const t = formatTimeLeft(message.unsendExpiresAt)}{#if t}
							<span class="ml-auto text-xs text-red-400 dark:text-red-500">{t}</span>{/if}{:else}
						<span class="ml-auto text-xs text-gray-400 dark:text-gray-600">(expired)</span>{/if}
				</button>
			{/if}
		</div>
