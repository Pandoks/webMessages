...
{#if !isGroup}
	...
	{#if p}
		{#if p.avatar}
			<img src={p.avatar} alt={p.name} class="shrink-0 rounded-full object-cover {singleSizes[size]}" />
		{:else}
			<div
				class="flex shrink-0 items-center justify-center rounded-full font-semibold text-white {singleSizes[size]} {getColor(p.name)}"
			>
				{getInitials(p.name) || '?'}
			</div>
		{/if}
	{:else}
		<div
			class="flex shrink-0 items-center justify-center rounded-full bg-gray-400 font-semibold text-white {singleSizes[size]}"
		>
			?
		</div>
	{/if}
{:else if displayParticipants.length === 2}
	...
	<div class="relative shrink-0 {containerSizes[size]}">
		...
		<div class="absolute left-0 top-0">
			{#if p1.avatar}
				<img src={p1.avatar} alt={p1.name} class="rounded-full border-2 border-white object-cover dark:border-gray-900 {dualSizes[size]}" />
			{:else}
				<div
					class="flex items-center justify-center rounded-full border-2 border-white font-semibold text-white dark:border-gray-900 {dualSizes[size]} {getColor(p1.name)}"
				>
					{getInitials(p1.name) || '?'}
				</div>
			{/if}
		...
		<div class="absolute bottom-0 right-0">
			{#if p2.avatar}
				<img src={p2.avatar} alt={p2.name} class="rounded-full border-2 border-white object-cover dark:border-gray-900 {dualSizes[size]}" />
			{:else}
				<div
					class="flex items-center justify-center rounded-full border-2 border-white font-semibold text-white dark:border-gray-900 {dualSizes[size]} {getColor(p2.name)}"
				>
					{getInitials(p2.name) || '?'}
				</div>
			{/if}
...
{:else if displayParticipants.length === 3}
	...
	<div class="relative shrink-0 {containerSizes[size]}">
		{#each displayParticipants as p, i}
			<div class="absolute {i === 0 ? 'left-1/2 top-0 -translate-x-1/2' : i === 1 ? 'bottom-0 left-0' : 'bottom-0 right-0'}">
				{#if p.avatar}
					<img src={p.avatar} alt={p.name} class="rounded-full object-cover {dualSizes[size]}" />
				{:else}
					<div class="flex items-center justify-center rounded-full font-semibold text-white {dualSizes[size]} {getColor(p.name)}">
						{getInitials(p.name) || '?'}
					</div>
				{/if}
			</div>
