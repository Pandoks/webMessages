<script lang="ts">
  ...
  const scheduledMessageGuidSet = $derived.by(
    () => new Set(scheduledMessages.map((msg) => msg.guid))
  );
  ...
  function canSaveInlineEdit(msg: Message): boolean {
    ...
    if (
      isScheduledPending(msg) &&
      editScheduleTimestamp != null &&
      editScheduleTimestamp <= Date.now() + 30_000
    ) {
      return false;
    }
  ...
  async function saveInlineEdit(msg: Message) {
    ...
    const beforeText = (msg.body ?? msg.text ?? '').trim();
    const afterText = editDraft.trim() || beforeText;
    const scheduledForMs = hasScheduleChange(msg)
      ? (editScheduleTimestamp ?? undefined)
      : undefined;
...
>
  ...
  {#each regularMessages as msg, i (msg.rowid)}
    ...
    {@const replyTo = msg.thread_originator_guid
      ? messageMap.get(msg.thread_originator_guid)
      : undefined}
