<script lang="ts">
  ...
  const preview = $derived.by(() => {
    ...
    const prefix = isScheduled ? 'Scheduled: ' : chat.last_message.is_from_me ? 'You: ' : '';
    const text = prefix + (body || (isScheduled ? 'Message' : ''));
    return text.length > 60 ? text.slice(0, 60) + '...' : text;
  });

  const dateStr = $derived(chat.last_message ? formatChatListDate(chat.last_message.date) : '');
...
<a
  ...
  class="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-gray-100
		{isActive ? 'bg-blue-50' : ''}"
>
  <ContactAvatar name={chat.display_name ?? '?'} identifier={avatarIdentifier} size={48} />
    ...
    <div class="flex items-baseline justify-between gap-2">
      <div class="flex min-w-0 items-center gap-1">
        ...
        {#if chat.is_pinned}
          ...
          >
            <path
              d="M16 3a1 1 0 0 1 .8 1.6L15.2 6.7l2.1 6.4a1 1 0 0 1-.95 1.31H13v5.2a1 1 0 0 1-1.71.7l-1.65-1.65a1 1 0 0 1-.29-.7v-3.55H7.05a1 1 0 0 1-.95-1.31l2.1-6.4L6.6 4.6A1 1 0 0 1 7.4 3z"
            />
          </svg>
        {/if}
      </div>
