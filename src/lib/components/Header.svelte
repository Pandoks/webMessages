...
<div class="flex items-center gap-3 border-b border-gray-200 bg-white px-4 py-3">
  ...
  <button
    ...
    class="mr-1 text-blue-500 md:hidden"
  ...
  <ContactAvatar name={chat.display_name ?? '?'} identifier={avatarIdentifier} size={36} />

  <div class="min-w-0 flex-1">
    <h2 oncontextmenu={openContextMenu} class="truncate text-base font-semibold text-gray-900">
...
{#if contextMenu}
  ...
  <div
    ...
    style="left: {Math.min(contextMenu.x, window.innerWidth - 188)}px; top: {Math.min(
      contextMenu.y,
