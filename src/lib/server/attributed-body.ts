...
function cleanupTypedstreamText(raw: string, fromAttributed: boolean): string {
  ...
  const hasTypedstreamMarkers =
    fromAttributed || raw.includes('iI') || s.includes('iI') || raw.includes('\ufffd');
...
function extractNSStringSegment(segment: Buffer): string {
  ...
  while (i < segment.length) {
    ...
    if (b < 0x80) {
      ...
      codePoint = ((b & 0x0f) << 12) | ((segment[i + 1] & 0x3f) << 6) | (segment[i + 2] & 0x3f);
      size = 3;
 ...
 */
export function getMessageText(text: string | null, attributedBody: Buffer | null): string {
